...
void suicide_burn(Vessel vessel) {
    ...
    auto thrust_stream = vessel.available_thrust_stream();
    ...
    while (h_speed_stream() > 10) {
        twr = thrust_stream() / (body_gravity * mass_stream());
    ...
    while (speed_stream() > FINAL_SPEED) {
        twr = thrust_stream() / (body_gravity * mass_stream());
        height_sb = get_height(twr, speed_stream(), body_gravity) + safe_margin;
        set_retrograde_direction(vessel, velocity_stream());
